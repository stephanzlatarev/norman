...
export default function() {
  ...
  while (merging) {
    merging = false;
    enemyWarriors.sort(function(a, b) {
      return (a.distance !== b.distance) ? (a.distance - b.distance) : (a.strength - b.strength);
    });
  ...
  let ownMilitaryStrength = Math.max(Math.random() * enemyMilitaryStrength * 2 - 0.01, 0.01);
  let ownDeploymentStrength = ownMilitaryStrength;
  for (const position of randomPositions(ALL_POSITIONS, SIZE * SIZE)) {
    const strength = Math.min(randomStrength(), ownMilitaryStrength);
    add(ownMilitary, position, strength);
    ownMilitaryStrength -= strength;
    if (ownMilitaryStrength < 0.01) break;
  }

  const deployment = [...EMPTY];
  for (const enemyWarrior of enemyWarriors) {
    const counterEnemyWarrior = enemyWarrior.strength * 2;
    if (ownDeploymentStrength < counterEnemyWarrior) break;
    add(deployment, enemyWarrior.position, counterEnemyWarrior);
    ownDeploymentStrength -= counterEnemyWarrior;
  }
