...
export default class Army extends Unit {
  ...
  async tock() {
    if (this.node.get("rally")) {
      ...
      this.node.set("orders", [{ abilityId: 16, targetWorldSpacePos: { x: locationX, y: locationY } }]);
      ...
      this.node.set("orders", [{ abilityId: 3674, targetWorldSpacePos: { x: locationX, y: locationY } }]);
    } else {
